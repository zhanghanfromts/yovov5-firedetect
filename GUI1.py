...
class MainWindow(QTabWidget):
    ...
    def __init__(self):
        ...
        self.webcam = True
        self.warning_threshold = 10 # 警报阈值
        self.continue_det_times = 0
        self.message = Message(self)
        self.message.signal.connect(self.box)
        self.countdown = Countdown(self)

        self.stopEvent.clear()
        self.model = self.model_load(weights="weights/best_origin.pt",
                                     device=self.device)  # todo 指明模型加载的位置的设备
        self.initUI()
        self.reset_vid()

    def box(self):
        select = QMessageBox.information(self, 'Warning', '有火灾！！！', QMessageBox.Ok)
    ...
    ***模型初始化***
    ...
    @torch.no_grad()
    def model_load(self, weights="'weights/best_origin.pt'",  # model.pt path(s)
    ...
    def detect_vid(self):
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                annotator = Annotator(im0, line_width=line_thickness, example=str(names))
                if len(det):
                    self.continue_det_times = self.continue_det_times + 1
                    if self.continue_det_times == self.warning_threshold:
                        LOGGER.info(f'有火灾，如果三分钟内无人处理，则进行自动报警!！')
                        self.message.start()
                        # self.countdown.start()
                    elif self.continue_det_times > self.warning_threshold:
                        LOGGER.info(f'有火灾，如果三分钟内无人处理，则进行自动报警!!')
                    # Rescale boxes from img_size to im0 size
                    det[:, :4] = scale_coords(im.shape[2:], det[:, :4], im0.shape).round()
                ...
                self.vid_img.setPixmap(QPixmap("results/single_result_vid.jpg"))
